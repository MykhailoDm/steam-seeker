...
class AppListDTOTest {
    ...
    void testAppListDTO() throws JsonProcessingException {
        ...
        AppListDTO appListDTO = new AppListDTO(appEntryDTOs);
        assertEquals(appEntryDTOs, appListDTO.getAppEntryDTOS());
        assertEquals("AppListDTO{appEntryDTOS=[AppEntryDTO{appId=" + appEntryDTO1.getAppId() + ", name='" + appEntryDTO1.getName() + "'}, AppEntryDTO{appId=" + appEntryDTO2.getAppId() + ", name='" + appEntryDTO2.getName() + "'}]}", appListDTO.toString());

        String jsonString = String.format("{\"applist\":[{\"appid\":%s,\"name\":\"%s\"}]}", appEntryDTO1.getAppId(), appEntryDTO1.getName());
        ObjectMapper objectMapper = new ObjectMapper();
        AppListDTO deserialized = objectMapper.readValue(jsonString, AppListDTO.class);
        AppEntryDTO actual = deserialized.getAppEntryDTOS().stream().findFirst().orElseThrow();
        ...
        appListDTO.setAppEntryDTOS(newAppEntryDTOS);

        AppEntryDTO actualAppEntry = appListDTO.getAppEntryDTOS().stream().findFirst().orElseThrow();
